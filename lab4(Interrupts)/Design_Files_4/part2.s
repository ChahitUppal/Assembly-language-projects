...
CONFIG_PRIV_TIMER:                             
				...
				mov		r3, #0xf
				STR      R1, [R0, #0x8]		//setting the interrupt mark bits
		 		ldr 	r3, =50000000	   // counter will be loaded with 50M 
		        str 	r3, [r1]			//adding load value
		
	            mov 	r3, #0b111	    // turn on A and E bits in counter control register
	            str 	r3, [r1,#8]		    // store in timer control reg
...
CONFIG_KEYS:                                    
				...
				mov		 r2, #0xF
				str		 r1, [r0, #0x8]		//setting interrupt mask bits
                MOV      PC, LR
 ...
 ***************************************************************************************/
                .global  KEY_ISR
KEY_ISR:        
                //chaning the value of run so that it stops counting
				LDR		r0, RUN
				EOR		r0, #1
				STR		r0, RUN
