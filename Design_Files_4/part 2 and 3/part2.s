...
/* Set up stack pointers for IRQ and SVC processor modes */
                  MSR	   CPSR_c, #0b11010010  //change modes and set stack pointer for this mode
				  LDR 	   SP, =0x20000
					
				  MSR 	   CPSR_c,  #0b11010011  //change modes and set stack pointer for this modes stack
				  LDR	   SP, =0x40000
...
CONFIG_PRIV_TIMER:                             
                LDR      R0, =0xFFFEC600                    // pushbutton timer address
             	MOV      R1, #0xF                                // set interrupt mask bits
             	STR      R1, [R0, #0x8]
				
				LDR 	 R2, =50000000
				STR		 R2, [R0] //set time to the value for the 0.25 counting
				MOV		 R2, #0b111   //now we set auto, enable and interupt to 1
				STR		 R2, [R0, #8]
...
IRQ_HANDLER:
                ...
                LDR      R4, =0xFFFEC100
                LDR      R5, [R4, #0xC]         // read the interrupt ID (R5 stores the interupt ID)

CHECK_KEYS:  CMP      R5, #73  //must add check to see if the clock interupted 
			 BLEQ	  KEY_ISR
			
CHECK_CLOCK: CMP 	  R5, #29
			 BLEQ	  PRIV_TIMER_ISR
		// if not recognized, stop here(how would we do the inf loop for this)
...
EXIT_IRQ:
             ...
             POP      {R0-R7, LR}
			   ...
			   SUBS     PC, LR, #4  //return (LR is actaully one line ahead)
...
KEY_ISR:        
				LDR R0, =0xFF200050
				MOV R2, #0xF
				STR R2, [R0, #0xC] //clear the interupt
				
				LDR R1, RUN
				EOR R1, #1 //flip what run is 
				STR R1, RUN
...
PRIV_TIMER_ISR:
				LDR R0, =0xFFFEC600
				MOV R2, #0x1
				STR R2, [R0, #0xC] //clear the interupt, reset the F bit
