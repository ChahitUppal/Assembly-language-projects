 ...
 ********************************************************************************/
			   ...
			   .equ      KEY_BASE,			0xFF200050 //base of key adress
_start:                                         
...
CONFIG_PRIV_TIMER:                             
				...
				mov		r3, #0xf
				STR      R3, [R1, #0x8]		//setting the interrupt mark bits
		 		ldr 	r3, =50000000	   // counter will be loaded with 50M 
		        str 	r3, [r1]			//adding load value
			 	        
                MOV      PC, LR
                   
/* Configure the pushbutton KEYS to generate interrupts */
CONFIG_KEYS:                                    
                LDR		 R0, =KEY_BASE     //push buttons base address
				mov		 r2, #0xF
				str		 r2, [r0, #0x8]		//setting interrupt mask bits
                MOV      PC, LR
...
IRQ_HANDLER:
				...
				CMP      R5, #73				//checking if key is interrupted
				BEQ		 KEY_ISR
				CMP		 R5, #29				//checking if timer interupt
				BEQ		 PRIV_TIMER_ISR
				
EXIT_IRQ:
                /* Write to the End of Interrupt Register (ICCEOIR) */
				//picked up from given part of part 1
                STR      R5, [R4, #0x10]
...
KEY_ISR:        
				...
				LDR		r0, =KEY_BASE		//getting the key address
				mov		r1, #0xf
				LDR      R3, [R0, #0xc]	    //storing teh edge cpature before clearing
				str		r1, [r0, #0xC] 		//clear the interupt 
				
KEY_0:			MOV		 R0, #KEY0     //getting code for key 0
				ANDS	 R0, R3, R0
				BEQ		 KEY_1
				LDR      R1, RUN		//load the current value
				EOR      R1, #1			//flip the run value
				STR      R1, RUN	
                B		 EXIT_IRQ		//go home
				
KEY_1:			MOV		 R0, #KEY1		//getting code for key 1
				ANDS	 R0, R3, R0
				BEQ		 KEY_2
				//now we increase the speed by 2
				...
				//increading load
				LDR		 R1, [R0]
				LSR		 R2, R1, #1		//doubling the current load
				STR		 R2, [R0]		//updating the load
				//starting the timer
				MOV      R1, #0b111			
				STR      R1, [R0, #0x8]
				B		EXIT_IRQ		//home time
				
KEY_2:			MOV		 R0, #KEY2		//getting code for key 2
				ANDS	 R0, R3, R0
				BEQ		 EXIT_IRQ
				//now we decrease the speed by 2
				LDR		 R0, =MPCORE_PRIV_TIMER
				//stopping the timer
				MOV      R1, #0			
				STR      R1, [R0, #0x8]
				//dec load
				LDR		 R1, [R0]
				LSL		 R2, R1, #1		//doubling the current load
				STR		 R2, [R0]		//updating the load
				//starting the timer
				MOV      R1, #0b111			
				STR      R1, [R0, #0x8]
 ...
 *****************************************************************************/
                .global    TIMER_ISR
PRIV_TIMER_ISR:
                //clearing the interupt now
				LDR		r0, =MPCORE_PRIV_TIMER
				mov		r1, #0x1
				str		r1, [r0, #0xC]		//resetting F bit
				//adding 1 to count if run is on [i.e 1]
				ldr		r0, RUN
				ldr		r1, COUNT
				ADD		r1,r1,r0
				STR		r1, COUNT
